...
pub use self::transports::{
    ...
    PostgresPandasTransport, RedshiftPandasTransport, SqlitePandasTransport,
};
...
pub fn write_pandas<'a>(
    ...
    match source_conn.ty {
        ...
        SourceType::Redshift => {
            let rt = Arc::new(tokio::runtime::Runtime::new().expect("Failed to create runtime"));
            let source = RedshiftSource::new(rt, &source_conn.conn[..])?;
            let dispatcher = Dispatcher::<_, _, RedshiftPandasTransport>::new(
                source,
                &mut destination,
                queries,
                origin_query,
